...
export const Link: React.FunctionComponent<{
  ...
  return <a style={{ textDecoration: 'none' }} className={`${className || ''}`} href={href} title={title}>{children}</a>;
};
...
export const ProjectLink: React.FunctionComponent<{
  ...
  return <Link href={`#?q=p:${value}`}>
    <span className={'project-name-label label-color-' + (projectNames.indexOf(projectName) % 6)}>
      {projectName}
    </span>
  </Link>;
};
