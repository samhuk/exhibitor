...
const determineServerPort = async (config: Config): Promise<GFResult<number>> => {
  ...
  const port = await findFreePort({
    ...
    events: {
      ...
      onAttemptFail: p => createGFError({ msg: c => `Port ${c.cyan(p.toString())} is not available to use. Trying next...` }).log({ customStackTraceRenderer: () => [] }),
      onAttemptUnexpectedError: (p, _err) => {
        err = createGFError({
          msg: c => `An unexpected error occured while determining whether port ${c.cyan(p.toString())} is available to use.`,
          inner: _err,
        })
        return { exit: true }
      },
...
const determineIntercomPort = async (host: string, serverPort: number): Promise<GFResult<number>> => {
  ...
  const port = await findFreePort({
    ...
    events: {
      ...
      onAttemptFail: p => logIntercomError(c => `Port ${c.cyan(p.toString())} is not available to use for ${NPM_PACKAGE_CAPITALIZED_NAME} Intercom. Trying next...`),
      onAttemptExcluded: p => logIntercomError(c => `Port ${c.cyan(p.toString())} is not available to use for ${NPM_PACKAGE_CAPITALIZED_NAME} Intercom because the ${NPM_PACKAGE_CAPITALIZED_NAME} Site Server is already using it.`),
      onAttemptUnexpectedError: (p, _err) => {
        err = createGFError({
          msg: c => `An unexpected error occured while determining whether port ${c.cyan(p.toString())} is available to use for ${NPM_PACKAGE_CAPITALIZED_NAME} Intercom.`,
          inner: _err,
        })
        return { exit: true }
      },
      onMaxAttempts: () => logIntercomError(c => `Max attempts exceeded for finding an available port to use for ${NPM_PACKAGE_CAPITALIZED_NAME} Intercom. Try using the ${c.bold(INTERCOM_PORT_ENV_VAR_NAME)} environment variable to explicitly set a different preferred port for ${NPM_PACKAGE_CAPITALIZED_NAME} Intercom.`),
    },
  })
